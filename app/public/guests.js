...
async function buscarInvitado() {
    ...
    if (invitado.length >= 1) {
        const asistio = invitado[0].asistencia_dia;
        acompanantesTemp = invitado[0].acompanantes_dia;
...
async function actualizarAcompanantes() {
    const numero = parseInt(document.getElementById("numero_empleado").value);
    const response = await fetch("http://localhost:4000/actualizarDia", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            numero_empleado: numero,
            acompanantes_dia: acompanantesTemp
        })
    });
